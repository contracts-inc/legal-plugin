...
- 選択肢付きの網羅的なヒアリング項目（共通7セクション＋契約類型13種・規制9領域の条件付き質問。取引形態の棚卸し結果から出す質問を自動選択）
...
│   └── skills/
│       ├── build-playbook/
│       │   ├── SKILL.md             # スキル定義（プレイブック構築・更新手順）
│       │   ├── references/
│       │   │   ├── interaction-format.md # 対話フォーマット規約（フォーム形式・引用禁止・出力量）
│       │   │   ├── playbook-schema.md   # 既定セクション構成・根拠の記載・匿名化要件
│       │   │   ├── evidence-sources.md  # 根拠情報の収集フロー（6系統）
│       │   │   ├── review-record-analysis.md # レビュー実績・版更新記録の分析（モードB-2）
│       │   │   ├── interview-guide.md   # 共通ヒアリング質問項目（選択肢付き）
│       │   │   ├── interview-guide-by-type.md # 契約類型別13種・規制別9領域の追加質問
│       │   │   ├── review-output.md     # レビュー用出力の体裁・観点の絞り方・FB取り込み
│       │   │   └── update-policy.md     # 改訂トリガー・差分提示・出力前セルフチェック
│       │   ├── scripts/
│       │   │   └── extract_review_feedback.py # Wordコメント・変更履歴・レビューシートの抽出
│       │   └── assets/
│       │       ├── interview-form.md    # 一括フォーム（記入用）の雛形
│       │       └── playbook-template.md # プレイブックの骨格テンプレート（汎用ダミー）
│       ├── review-contract/
...
本スキルは**カスタマイズせずにそのまま利用する前提**です。ヒアリング質問項目（共通7セクション＋契約類型13種・規制9領域の追加質問）、実績集計や改訂候補判定のしきい値、改訂トリガー、出力前セルフチェックの項目はスキル内に既定値として作り込まれており、組織固有の運用はヒアリングで聴取して反映します。
