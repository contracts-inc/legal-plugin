...
- 3つのモード（新規構築 / 更新・改訂（規程・方針ドリブン） / 更新・改訂（レビュー実績ドリブン））。出力前の法令適合性チェックはA/B共通で必ず実施（e-Govコネクタが使えない場合は有効化を促す）
- 「棚卸ししたい」という依頼も更新・改訂として受け付け、改訂候補0件なら「改訂不要」と報告して終了
- 収集前に「何をインプットにするか」を全モードで必ずフォームで確認（改訂のきっかけ・手元の資料・参照可否・コネクタの接続状況。2回目以降や実績ドリブンの改訂でも省略しない）
...
- スキルが定義するのは全体の流れ・モード・インプット・アウトプットのみ。収集や集計の手順、ヒアリングの設問、品質面の点検項目は定義せず、出力仕様を満たすようにモデルが判断する（契約実務の一般知識をスキルに列挙しない）
...
- すべての質問・判断依頼をフォーム形式に統一（番号で回答、常に「保留」を用意、1フォーム7問まで、確認事項が20件以上なら記入用フォームをファイルで配布）
- 回答の受け取り方を統一（ランタイムに選択式の質問UIがあればそれを使い、無ければMarkdownフォーム。1回のやり取りで両方を混ぜない。数値の記入欄や複数選択が必要な設問だけMarkdownフォームに切り替える）
- 質問文の自己完結（現行プレイブック・規程・レビュー記録・法令の引用や条番号・節番号の参照を質問に含めず、判断材料は件数などの数値のみ）
- セクション・契約類型・立場ごとに1つずつ完結させる進行と、長い一覧（実績集計・分析結果・差分・確認事項・本文）のファイル出力
...
- レビュー形式（Word / 表計算ソフト用のCSV / チャット上の表）をファイル作成前にユーザーに選ばせ、選ばれた1形式だけを渡す（複数形式を同時に渡さない。選ばれた形式を生成できない場合のみ次の形式に切り替え）。`review-contract` 連携用の `playbook.md` は承認確定後に出力
...
│       │   ├── references/
│       │   │   ├── interaction-format.md # 対話の規約（フォーム形式・回答スケール・引用禁止・出力量）
│       │   │   ├── playbook-schema.md   # アウトプット定義（既定セクション構成・根拠・匿名化・分割規則）
│       │   │   ├── evidence-sources.md  # インプット定義（6系統・規制フック・CLMの制約）
│       │   │   ├── review-output.md     # レビュー用出力の体裁・観点の絞り方・FB取り込み
│       │   │   └── update-policy.md     # 改訂トリガー・差分提示・法令適合性チェック・実績分析
│       │   ├── scripts/
...
本スキルは**カスタマイズせずにそのまま利用する前提**です。スキルが定義しているのは全体の流れ・モード・インプット（6系統と規制フック）・アウトプット（プレイブックの構成とレビュー用出力）に限られ、ヒアリングの設問や集計の手順は意図的に定義していません。組織固有の運用はヒアリングで聴取して反映します。凍結した質問票が必要な場合は、組織側の資産として別に管理してください。
...
- 交渉の優先順位（Tier 1〜3）
- 修正案のフォーマット
- メタデータブロック（版数・最終更新日時・承認者・承認日・次回見直し予定・対象範囲）と改訂履歴（§6）
