...
- `review-contract` スキルに「5. レビュー補足資料の添付」セクションを新規追加。ユーザーが希望する場合のみ実行する任意ステップとして、レビュー報告書・指摘一覧・チェックリスト等を`get_upload_url`（返却された`curl_command`を実行）と`upload_attachment`ツールでレビュー対象の契約書の添付ファイルとして登録するフローを定義。契約書本文（docx）へのサマリー追加禁止ルールとの関係を明記し、本文の指摘はWordコメント、全体サマリーが必要な場合は添付ファイルとして別ファイルで登録する方針を示した。既存の「5. レビュー完了コメントの投稿」は「6.」、「6. レビュータスクの完了」は「7.」に番号を繰り上げ。
...
- 添付ファイル操作の制約・注意事項を明記。`download_attachment`のダウンロードURLは有効期限が300秒（5分）で期限切れの場合は再取得する、`upload_attachment`は25MBを超えるファイルを登録できない、`list_attachment_files`はコメントに添付されたファイルを含まない、契約書の閲覧権限がない場合は一覧が空・ダウンロードが403となる、契約書・契約書コメント・作成依頼タスクコメント以外のコメントの添付ファイルは404となり再試行しない。
