...
- 交渉担当者向けスキル第5弾（最終） `obligation-monitor`（UC5）を追加。締結後の契約群をCLMから横断取得し、更新期限・自動更新・義務履行期日をHTMLレポートで俯瞰。特定条項（制裁・自動更新・独占期間など）の横断抽出と週次定期実行に対応。UC1〜UC5の全フェーズ（受領〜締結〜履行）が揃った。

## [1.4.0] - 2026/07/15

### Added

- 交渉担当者向けスキル第4弾 `approval-brief`（UC4）を追加。契約書と案件数値から稟議サマリ（採算・最悪シナリオ・撤退条件・主要リスクTop3）を1枚に自動生成。契約条項の「経営指標への翻訳」を行い、clause-review・finance-impactの結果も取り込める。

## [1.3.0] - 2026/07/15

### Added

- 交渉担当者向けスキル第3弾 `finance-impact`（UC3）を追加。締結前の契約書から会計・税務インパクトに効く条項を抽出し、経理財務向け「経理確認依頼シート」を生成・CLMで共有。締結直前の経理差し戻しを予防する。
